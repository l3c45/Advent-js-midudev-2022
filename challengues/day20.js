...
function howManyReindeers(reindeerTypes, gifts) {
  const a = gifts.map((pais) => {
    let tempWeigth=pais.weight
  return   reindeerTypes.reduce((equipo, reno, i, arr) => {
      if (reno.weightCapacity <= pais.weight) {
        if(tempWeigth>reno.weightCapacity){
          equipo.push(reno);
        tempWeigth-reno.weightCapacity
        return equipo;}
      } else {
        return equipo;
      }
