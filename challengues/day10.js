...
function checkJump(heights) {
  const simplificado = heights.reduce((acc, val, i, arr) => {
    if (i === 0) {
      acc.push(val);
      return acc;
    } else if (val === arr[i - 1]) {
  ...
  let subiendo = true;
  const check=simplificado.reduce((acc, val, i, arr) => {
    if (arr.length === 1) return false;
    if (i === 0) {
      return acc;
    } else if (val >= arr[i - 1] ) {
      if (val > arr[i + 1]) subiendo = false;
      return acc;
    } else if (!subiendo && (val > arr[i + 1] || i === arr.length - 1)) {
      return acc;
    } else {
      return false;
    }
