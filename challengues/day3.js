...
function distributeGifts(packOfGifts, reindeers) {
    const weigthGifts = packOfGifts.reduce((acc, val) => acc + val.length, 0);
    const weigthReindeer = reindeers.reduce(
      (acc, val) => acc + val.length * 2,
      0
    );
    const amount = Math.floor(weigthReindeer / weigthGifts);
    return amount;
  }